...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands);
		int found_command = -1;
		int matched_command = 0;
		int not_valid_args = 0;
		int z;
		for (z = 0; z < NUM_OF_COMMANDS; z++){ // help == help
			int s= number_of_arguments -1;
			if(strcmp(arguments[0], commands[z].name) == 0){ // mfrood btcheck 3l name of command mwgood 3ndy fl array of struct wla l2
				if(s == commands[z].num_of_args||commands[z].num_of_args == -1){
					found_command = z;
					break;
				}
				else{
					LIST_INSERT_HEAD(&foundCommands,&commands[z]);
					not_valid_args = 1;
					break;
				}
			}
		...
		for (z = 0; z < NUM_OF_COMMANDS; z++) {
			int x,i=0;
			int y,a;
			int s = strlen(commands[z].name);
			int len = strlen(arguments[0]);
			x=0;
			int p = 0;
			for (y = 0; y < len; y++){ // c l k
				for (a = p; a < s; a++){ //
					if(arguments[0][y] == commands[z].name[a]){
						x++;
						p = y;
						break;
					}
