...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 last_add = ROUNDUP(virtual_address+size,PAGE_SIZE);
	for(uint32 i =virtual_address ;i <last_add ;i+=PAGE_SIZE)
	{
...
//	return;
	...
	uint32 start_add = virtual_address;
	uint32 last_add = ROUNDUP(virtual_address+size,PAGE_SIZE);
	for(uint32 i = start_add ; i <last_add ; i+=PAGE_SIZE)
	{
