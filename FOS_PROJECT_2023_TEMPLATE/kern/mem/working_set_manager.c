...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	//if(LIST_SIZE(&(e->page_WS_list)) <e->page_WS_max_size)
	    struct WorkingSetElement *working_set_element;
	   // cprintf("creating elemnt....\n");
		working_set_element =(struct WorkingSetElement *)kmalloc(sizeof(struct WorkingSetElement));
		if(working_set_element!= NULL)
		{
		    //cprintf(" elemnt ! null ....\n");

			working_set_element->virtual_address = virtual_address;
			//cprintf(" returning elemnt ....\n");
			//cprintf("va param = %x\n",virtual_address);
			//cprintf("va of ws = %x\n",working_set_element->virtual_address);
			return working_set_element;

		}
		else
		{
			panic("Failed to allocate memory for working set element");
		}
