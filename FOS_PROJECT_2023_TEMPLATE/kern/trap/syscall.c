...
void* sys_sbrk(int increment)
{
	...
	struct Env* env = curenv; //the current running Environment to adjust its break limit
	    uint32 round_size = ROUNDUP(increment, PAGE_SIZE);

		uint32 noPages = round_size/PAGE_SIZE;
		if(increment > 0 && (env->brk)+increment <= env->hardl){
             uint32 oldbrk = env->brk;
        	 allocate_user_mem(env, env->brk, round_size);
		...
		else if(increment < 0 && (env->brk+increment) >= env->start){//2l test m4 byd5ol hna 2slan
            increment *= -1;
			sys_free_user_mem(ROUNDUP(env->brk,PAGE_SIZE),ROUNDDOWN(increment,PAGE_SIZE));

		/*	for(uint32 i = env->brk; i < env->brk - (noPages*PAGE_SIZE); i -= PAGE_SIZE)
			{
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_FREE:
		 ...
		 if(a1 == (uint32)NULL || a1 +a2>= (uint32)USER_LIMIT)
		 { cprintf("==========ms1.1===========/n");
	...
	case SYS_ALLO:
		...
		if( a1 == (int)NULL || a1+a2 >= (int)USER_LIMIT)
		{
			 cprintf("==========ms1.2===========/n");
			sched_kill_env(curenv->env_id);
		}
			//else if (a1>USER_LIMIT - PAGE_SIZE)
						  // sched_kill_env(curenv->env_id);
